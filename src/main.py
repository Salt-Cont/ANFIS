...
def распознать():
    if Путь:
        ...
        СписокМоделей = []
        СписокМоделей.append(загрузитьСписокНаборов())
...
def обучать():
    ...
    СписокНаборов = []
    СписокНаборов.append(загрузитьСписокНаборов())
...
def загрузитьДанныеДляОбучения():
    global Путь
    global Набор
    Путь = filedialog.askopenfilename(filetypes=[("Изображения", "*.png;*.jpg;*.jpeg")])
    Набор = загрузитьНабор(Путь)
...
КнопкаЗагрузкиДанныхДляОбучения = Button(ПолеИзображения, text=f"Загрузить данные для обучения", command=загрузитьДанныеДляОбучения)
